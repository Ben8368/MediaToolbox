...
- 内部开发没有代码级阻断；**TD-031** 依赖告警已完成在线分级与兼容修复，React Router 剩余告警不适用于当前 `BrowserRouter` 模式，Electron Builder 稳定版的构建期上游例外转为 **TD-033**。公开发布仍被 **TD-023** 阻断：缺少根 `LICENSE`，且 `assets/web-composer/PROVENANCE.json` 尚未逐项覆盖 8 个基础 MP4 与 3 个补充 MP4，详见 [TECH_DEBT.md](docs/TECH_DEBT.md)。
- **TD-019** 的实现与自动化烟测已接入，尚待首次 tag Release 在 Windows、macOS、Linux 实跑；这不是“renderer 不可用”的代码阻断。
- 最近客观验证：2026-07-30 `npm run verify` 通过（47 个测试文件、277 项）；依赖兼容补丁后的 macOS arm64 Electron 目录包构建、包内 API、根 renderer、图标和代表性视频 loopback 检查通过。`npm run release:preflight:public` 仍按设计拒绝缺失的公开授权证据。
