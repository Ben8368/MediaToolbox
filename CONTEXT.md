...
> **最近更新：** 2026-07-04，非验收类黄灯继续收口：受控上传文件选择与桌面确认流、下载 app 双通道入口、资产索引、任务中心统一 jobs 取消、浏览器下载速率采样和 PSD 模板检查工作台已接入；真实桌面体验与生产打包验收后续统一处理。
...
- Browser Network 第一版已覆盖隔离 session、下载事件、稳定 ID 回写、工作区上传文件选择确认、权限审计和 API/jobs 契约；下载 app 已提供媒体解析/浏览器资源双通道入口；弹窗细化、标签页和生产打包资源加载仍需后续体验验收。
