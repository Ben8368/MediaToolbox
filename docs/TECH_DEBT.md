...
- **验证方式：** 为 `manifest.json` 的 8 个基础 MP4 与 `supplemental.json` 的 1 个补充 MP4 逐项记录原始来源、版权方、许可证或书面授权及兼容范围，并确认仓库 `LICENSE` 与产品发布范围一致
...
- **问题：** 基础 PNG 和 1 秒 1080p H.264 MP4 已通过；视频加载失败会明确终止导出，录制成功或异常路径均会释放 recorder、stream track、媒体监听器与 timer；8 个基础视频和 1 个补充视频均已固定来源与 SHA-256，但 4K/长时捕获基线仍待完成
