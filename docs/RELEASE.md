...
- 当前拆分范围是 8 个 Release Asset 基础 MP4 与 3 个固定上游 HTTPS URL、大小和 SHA-256 的补充 MP4：源码仓库保留字体、CSS、必要图片、两份清单、操作说明和安装工具，但不跟踪视频文件。
...
- 当前仓库和视频尚无完整的公开分发许可证/来源清单，因此只允许开发和内部候选构建；公开产品发布必须先补齐根 `LICENSE` 与 `assets/web-composer/PROVENANCE.json`，逐项覆盖基础与补充清单当前合计 11 个视频，`npm run release:preflight:public` 会阻止证据缺失的 tag Release。
