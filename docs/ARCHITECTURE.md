...
- 所有长任务进入统一 Job 模型，支持进度、日志、取消与失败状态呈现。API 启动会将持久库遗留的活动任务安全标记为 `failed`、记录重启中断原因并吊销绑定授权；自动重试、断点续跑和统一 executor 安全关闭屏障尚未实现，后续引入时必须先定义 attempt、幂等输出、调度策略、退避与可观测性边界。
