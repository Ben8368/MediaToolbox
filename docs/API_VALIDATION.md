...
6. 勾选“去除背景”后，分别导出 6 个预设的 PNG；确认每张文件保留前景内容、背景 Slot 与 Trace Grid 背景遮罩均不出现，且在透明棋盘格查看器中显示 Alpha。未勾选时保持原有背景导出。
...
9. 打开 `/preset/`，确认重定向到默认预设；使用顶部“选择预设”切换全部 6 个独立预览页，并检查 `16:9`、`4:3`、`1:1`、`9:16` 画幅控件。
