...
4. 切换全部 8 个预设，确认选中状态不会跨预设残留；多开工作台时，选择、坐标和捕获消息按 source、origin、session、预设 ID/版本隔离。
5. 切换 `16:9`、`9:16`、`1.91:1`、`1:1.91`、`4:3`、`3:4`、`1:1` 与分辨率，确认 iframe 的真实像素尺寸随设置变化，外层窗口只改变显示缩放，设计字号与偏移按画布比例呈现。
6. 勾选“去除背景”后，分别导出 8 个预设的 PNG；确认每张文件保留前景内容、背景 Slot 与 Trace Grid 背景遮罩均不出现，且在透明棋盘格查看器中显示 Alpha。未勾选时保持原有背景导出。
...
9. 打开 `/preset/`，确认重定向到默认预设；使用顶部“选择预设”切换全部 8 个独立预览页，并检查全部 7 种画幅控件。
