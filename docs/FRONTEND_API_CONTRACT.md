...
- `JobRecord` 除基础状态外返回 `attempt`、`maxAttempts`、稳定 `outputToken` 与可选 `nextAttemptAt`。下载和转码任务默认 `maxAttempts=3`；仅当 adapter 将错误标记为 `retryable` 时，任务才从 `running` 回到带 `nextAttemptAt` 的 `queued`，随后按 1、2 秒退避再次执行。其余任务当前 `maxAttempts=1`。下载/转码的版本化执行载荷只存于 API 私有数据库表，不进入 `JobRecord` 或任何 Web 响应。
- 自动重试不新增 `retrying` 枚举，也不把一次 attempt 失败视为 Job 终态；最终成功、次数耗尽、用户取消或正常服务关闭后才执行终态资源清理。API 异常重启会恢复载荷完整且仍有剩余 attempt 的下载/转码任务，并沿用原 `outputToken`；缺少/损坏载荷、次数耗尽、暂停任务以及 PSD/Web Composer 等未开放恢复的任务会明确失败，不会盲目重跑。
...
- Web Composer 不接收客户端输出路径；服务端以 `packages/contracts` 的预设 catalog 精确校验当前 8 个 ID/版本元组（`multi-showcase@2`、`trace-grid@1`、`vex-vision@1`、`wandor@1`、`foundation@1`、`lumora@2`、`vaultshield@2`、`viktor@2`），并校验 PNG/WebM 文件签名、体积和捕获元数据；`videoFormat` 仅允许 `mp4` 或 `mov-alpha`，输出文件名与 `/Workspace/Exports` 路径完全由服务端生成。旧版、未知预设或格式组合返回 400。
