...
- `POST /api/fetch/tasks` 仅接受共享 `FetchTaskDraft` 字段：工作区内 `output_dir`、字幕/字幕格式、H.264/转码偏好、支持的浏览器 Cookie 来源及 `1–4` 的单批次并发。服务端全局并发也限制为 `4`；未知字段、越界路径或超界并发返回 4xx，绝不静默忽略。
